...
def render(x: np.ndarray, y: np.ndarray, frames, best: tuple[float, float]) -> None:
    ...
    # Draw ALL sales faintly (matching the "Past home sales" slide) and put red
    # error bars on a readable subset only. The fit and SSE use the full data.
    ...
    with writer.saving(fig, str(OUT_GIF), dpi=110):
        for a, b, sse in frames:
            ax.clear()
            # all sales, faint
            ax.scatter(x, y, s=14, color="#1f4e79", alpha=0.16,
                       edgecolor="none", zorder=2, label="Past sales (all)")
            # error segments in red, on a readable subset
            for xi, yi in zip(xs, ys):
                ax.plot([xi, xi], [yi, a * xi + b], color="#d62728", lw=1.0, alpha=0.7)
            ax.scatter(xs, ys, s=26, color="#1f4e79", zorder=3)
